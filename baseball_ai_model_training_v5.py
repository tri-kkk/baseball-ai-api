# baseball_ai_model_training_v5.py
"""
Baseball AI 예측 모델 학습 스크립트 v5

변경사항 (v4 대비):
- 선발투수 feature 를 3리그(MLB/KBO/NPB) 전부 활성화
  (v4 는 MLB 만 투수 포함, KBO/NPB 제외였음 → 이제 데이터가 채워져 전리그 사용)
- 투수 결측은 리그 평균(PITCHER_DEFAULTS)으로 대체 (서빙 predict 라우트와 동일 규칙)
- 그 외 구조/모델(GradientBoosting)·시계열 분할(shuffle=False)은 v4 유지
"""
...
parser.add_argument('--league', type=str, default='ALL', choices=['MLB', 'KBO', 'NPB', 'ALL'])
...
# v5: 모든 리그가 BASE + 투수 feature 사용
FEATURE_COLUMNS_BY_LEAGUE = {
    'MLB': BASE_FEATURE_COLUMNS + PITCHER_FEATURES,
    'KBO': BASE_FEATURE_COLUMNS + PITCHER_FEATURES,
    'NPB': BASE_FEATURE_COLUMNS + PITCHER_FEATURES,
}

# 투수 결측 대체값 (서빙 predict 라우트와 동일: era 4.20 / whip 1.30 / k 150)
PITCHER_DEFAULTS = {'era': 4.20, 'whip': 1.30, 'k': 150}
...
def train_league(league: str):
    ...
    # v5: 전 리그 투수 null 처리 + 파생 feature
    df_features['home_pitcher_era'] = df_features['home_pitcher_era'].fillna(PITCHER_DEFAULTS['era'])
    df_features['away_pitcher_era'] = df_features['away_pitcher_era'].fillna(PITCHER_DEFAULTS['era'])
    df_features['home_pitcher_whip'] = df_features['home_pitcher_whip'].fillna(PITCHER_DEFAULTS['whip'])
    df_features['away_pitcher_whip'] = df_features['away_pitcher_whip'].fillna(PITCHER_DEFAULTS['whip'])
    df_features['home_pitcher_k'] = df_features['home_pitcher_k'].fillna(PITCHER_DEFAULTS['k'])
    df_features['away_pitcher_k'] = df_features['away_pitcher_k'].fillna(PITCHER_DEFAULTS['k'])
...
print(f"Baseball AI 모델 학습 v5")
print(f"리그: {LEAGUE} | 최소경기: {MIN_GAMES}")
print(f"  전 리그 투수 feature 포함 ({len(FEATURE_COLUMNS_BY_LEAGUE['MLB'])}개)")
